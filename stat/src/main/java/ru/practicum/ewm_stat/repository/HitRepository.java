package ru.practicum.ewm_stat.repository;
...
public interface HitRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query(value = "select new ru.practicum.ewm_stat.model.ViewStats(e.app, e.uri, count(e.ip)) " +
    ...
    @Query(value = "select new ru.practicum.ewm_stat.model.ViewStats(e.app, e.uri, count(distinct e.ip)) " +
            "from EndpointHit as e " +
            "where e.timestamp between :start and :end " +
            "and (coalesce(:uris, null) is null or e.uri in :uris) " +
            "GROUP BY e.uri, e.app")
